...
type View = "home" | "terms" | "cards" | "quiz";
type Card = {
  id: number;
  unit: string;
  question: string;
...
function createQuiz(unit: string, importantOnly: boolean, count = 10): QuizQuestion[] {
...
export default function Home() {
  ...
  function startQuiz(unit = selectedUnit, important = importantOnly) {
    ...
    setQuiz(createQuiz(unit, important));
    setQuizIndex(0);
      ...
      <main>
            ...
            <section className="hero-grid">
              <div className="hero-copy">
                ...
                <div className="hero-actions">
                  <button className="button primary" onClick={() => startQuiz("すべて", true)}>今日の10問を始める <span>→</span></button>
        ...
        {view === "quiz" && (
          <div className="page content-page quiz-page">
            <PageTitle kicker="EXAM PRACTICE" title="4択クイズ" description="本番を意識して、10問をテンポよく。間違えた問題は自動で復習リストへ入ります。" />
            {quiz.length === 0 ? (
              <div className="quiz-setup">
                <div className="setup-copy"><span>10 QUESTIONS</span><h2>出題範囲を選ぶ</h2><p>まずは重要度★★★から始めるのがおすすめです。</p></div>
                <div className="setup-options"><UnitSelect value={selectedUnit} onChange={changeUnit} /><label className="toggle"><input type="checkbox" checked={importantOnly} onChange={(event) => setImportantOnly(event.target.checked)} /><span /> ★★★だけ</label><button className="button primary" onClick={() => startQuiz()}>クイズを開始 →</button></div>
              </div>
            ) : quizDone ? (
              <div className="quiz-result">
                ...
                <h2>{quizScore === quiz.length ? "全問正解、完璧です！" : quizScore >= 8 ? "かなり仕上がっています！" : "間違いは、伸びしろです。"}</h2>
                ...
                <div className="question-panel">
                  ...
                  <div className="choice-grid">
                    {quiz[quizIndex].choices.map((choice, index) => {
                      ...
                      return <button key={choice} className={`choice ${correct ? "correct" : ""} ${wrong ? "wrong" : ""}`} onClick={() => answerQuiz(choice)} disabled={Boolean(selectedAnswer)}><span>{String.fromCharCode(65 + index)}</span><b>{choice}</b>{correct && <i>✓</i>}{wrong && <i>×</i>}</button>;
                    })}
                  </div>
                  {selectedAnswer && (
                    <div className={`feedback ${selectedAnswer === quiz[quizIndex].answer ? "good" : "bad"}`}><div><b>{selectedAnswer === quiz[quizIndex].answer ? "正解！" : "惜しい！正解を確認しよう"}</b>{quiz[quizIndex].note && <p>ポイント：{quiz[quizIndex].note}</p>}</div><button onClick={nextQuizQuestion}>{quizIndex + 1 === quiz.length ? "結果を見る" : "次の問題"} →</button></div>
                  )}
                </div>
              </div>
            )}
